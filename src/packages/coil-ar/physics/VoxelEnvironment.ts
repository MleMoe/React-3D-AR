...
export class VoxelEnvironment {
  ...
  length: Vector3;
  precision: number;
  grid: Array<VoxelBody>;

  constructor(world: World, size: number, precision: number) {
    if (size === undefined) {
      size = 5.0;
    }
     ...
     */
    this.shape = new Box(
      // new Vec3(-precision, -precision, -precision)
      new Vec3(precision, precision, precision)
    );
